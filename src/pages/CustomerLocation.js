...
import CustomerLocationTable from "components/customerLocation/CustomerLocationTable";
import SelectCategory from "components/form/SelectCategory";
import MainDrawer from "components/drawer/MainDrawer";
import CustomerLocationDrawer from "components/drawer/CustomerLocationDrawer";
import CheckBox from "components/form/CheckBox";
import useCustomerLocationFilter from "hooks/useCustomerLocationFilter";
...
const CustomerLocations = () => {
  ...
  const { data, loading } = useAsync(() =>
    CustomerLocationServices.getAll({
      page: currentPage,
      limit: limitData,
      // category: category,
      title: searchText,
  ...
  } = useCustomerLocationFilter(data?.products);

  return (
    <>
      <PageTitle>{t("CustomerLocationsPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="CustomerLocations" />
      <MainDrawer>
        <CustomerLocationDrawer id={serviceId} />
      </MainDrawer>
          ...
          >
            <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
              <Input
                ...
                type="search"
                name="search"
                placeholder="Search CustomerLocation"
              />
              <button
                type="submit"
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="CustomerLocation Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="CustomerLocation" />
      )}
    </>
  );
};
