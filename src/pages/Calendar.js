...
const Calendars = () => {
  ...
  } = useCalendarFilter(data?.products);

  return (
    <>
      <PageTitle>{t("CalendarsPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="Calendars" />
      <MainDrawer>
        <CalendarDrawer id={serviceId} />
      </MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="Calendar Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="Calendar" />
      )}
    </>
  );
};
