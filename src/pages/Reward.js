...
import RewardDrawer from "components/drawer/RewardDrawer";
...
const Rewards = () => {
  ...
  } = useRewardFilter(data?.products);

  return (
    <>
      <PageTitle>{t("RewardsPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="Rewards" />
      <MainDrawer>
        <RewardDrawer id={serviceId} />
      </MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="Reward Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="Reward" />
      )}
    </>
  );
};
