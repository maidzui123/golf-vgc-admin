...
import ClubPostTable from "components/clubPost/ClubPostTable";
import SelectCategory from "components/form/SelectCategory";
import MainDrawer from "components/drawer/MainDrawer";
import ClubPostDrawer from "components/drawer/ClubPostDrawer";
import CheckBox from "components/form/CheckBox";
import useClubPostFilter from "hooks/useClubPostFilter";
...
const ClubPosts = () => {
  ...
  } = useClubPostFilter(data?.products);

  return (
    <>
      <PageTitle>{t("CLUB'S POST REPORTS MANAGEMENT")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="ClubPosts" />
      <MainDrawer>
        <ClubPostDrawer id={serviceId} />
      </MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="ClubPost Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="ClubPost" />
      )}
    </>
  );
};
