...
const Flights = () => {
  ...
  } = useFlightFilter(data?.products);

  return (
    <>
      <PageTitle>{t("Flights Page")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="Flights" />
      {/* <MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          <Table>
            <TableHeader>
              <tr>
                ...
                <TableCell className="text-left">{t("Players Number")}</TableCell>
          ...
          <TableFooter>
            <Pagination
              ...
              label="Flight Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="Flight" />
      )}
    </>
  );
};
