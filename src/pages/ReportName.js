...
import ReportNameTable from "components/reportName/ReportNameTable";
import SelectCategory from "components/form/SelectCategory";
import MainDrawer from "components/drawer/MainDrawer";
import ReportNameDrawer from "components/drawer/ReportNameDrawer";
import CheckBox from "components/form/CheckBox";
import useReportNameFilter from "hooks/useReportNameFilter";
...
const ReportNames = () => {
  ...
  } = useReportNameFilter(data?.products);

  return (
    <>
      <PageTitle>{t("ReportNamesPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="ReportNames" />
      <MainDrawer>
        <ReportNameDrawer id={serviceId} />
      </MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="ReportName Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="ReportName" />
      )}
    </>
  );
};
