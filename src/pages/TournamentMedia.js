...
import TournamentMediaTable from "components/tournamentMedia/TournamentMediaTable";
import SelectCategory from "components/form/SelectCategory";
import MainDrawer from "components/drawer/MainDrawer";
import TournamentMediaDrawer from "components/drawer/TournamentMediaDrawer";
import CheckBox from "components/form/CheckBox";
import useTournamentMediaFilter from "hooks/useTournamentMediaFilter";
...
const TournamentMedias = () => {
  ...
  const { data, loading } = useAsync(() =>
    TournamentMediaServices.getAll({
      page: currentPage,
      limit: limitData,
      // category: category,
      title: searchText,
  ...
  } = useTournamentMediaFilter(data?.products);

  return (
    <>
      <PageTitle>{t("TournamentMediasPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="TournamentMedias" />
      <MainDrawer>
        <TournamentMediaDrawer id={serviceId} />
      </MainDrawer>
          ...
          >
            <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
              <Input
                ...
                type="search"
                name="search"
                placeholder="Search TournamentMedia"
              />
              <button
                type="submit"
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="TournamentMedia Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="TournamentMedia" />
      )}
    </>
  );
};

export default TournamentMedias;
