...
const Tournaments = () => {
  ...
  const { data, loading } = useAsync(() =>
    TournamentServices.getAll(queryParams)
  );
  ...
  const { serviceData } = useTournamentFilter(data?.data);
  // const exportToExcel = () => {
  ...
  const translateFieldNames = (fieldName) => {
    // Add your translation mapping here
    const translations = {
      createdAt: "Ngày tạo",
      updatedAt: "Ngày cập nhật",
      start_date: "Ngày bắt đầu",
      end_date: "Ngày kết thúc",
      register_expired: "Ngày hết hạn đăng ký",
      name: "Tên giải đấu",
      description: "Mô tả",
      status: "Trạng thái",
      category: "Thể loại",
  ...
  const exportToExcel = async () => {
    const allTours = await TournamentServices.getAll({
      page: 1,
    ...
    if (allTours.data && allTours.data.length > 0) {
      const formattedData = allTours.data.map((tournament) => {
        const { _id, ...formattedTournament } = tournament;

        Object.keys(formattedTournament).forEach((field) => {
          if (
            field === "start_date" ||
            field === "end_date" ||
            field === "register_expired" ||
            field === "createdAt" ||
            field === "updatedAt"
          ) {
            formattedTournament[field] = moment(
              formattedTournament[field]
            ).format("DD/MM/YYYY");
          } else if (field === "phone") {
            formattedTournament[field] = formatPhone(
              formattedTournament[field]
            );
          }

          formattedTournament[translateFieldNames(field)] =
            formattedTournament[field];
          delete formattedTournament[field];
        });
  ...
  return (
    <>
      ...
      ></link>
      <PageTitle>{t("TOURNAMENTS MANAGEMENT")}</PageTitle>
            ...
            <div className="lg:flex md:flex xl:justify-end xl:w-1/2  md:w-full md:justify-start flex-grow-0">
              <div className="w-full md:w-32 lg:w-32 xl:w-32 mr-3 mb-3 lg:mb-0">
                <Button
                  ...
                  className="w-full rounded-md h-10 bg-red-300 disabled btn-red"
                >
                  <span className="mr-2">
                    <FiTrash2 />
                  </span>
          ...
          <form onSubmit={handleSearchSubmit} className="py-3">
            <div className="flex flex-col md:flex-row items-center">
              ...
              <Input
                ...
                type="search"
                name="search"
                placeholder={t("Search Tournament")}
              />
              <Button
                type="submit"
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="Tournament Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title={t("Tournament")} />
      )}
    </>
  );
};
