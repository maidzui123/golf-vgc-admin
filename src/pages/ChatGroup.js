...
import ChatGroupTable from "components/chatGroup/ChatGroupTable";
import SelectCategory from "components/form/SelectCategory";
import MainDrawer from "components/drawer/MainDrawer";
import ChatGroupDrawer from "components/drawer/ChatGroupDrawer";
import CheckBox from "components/form/CheckBox";
import useChatGroupFilter from "hooks/useChatGroupFilter";
...
const ChatGroups = () => {
  ...
  } = useChatGroupFilter(data?.products);

  return (
    <>
      <PageTitle>{t("ChatGroupsPage")}</PageTitle>
      <DeleteModal ids={allId} setIsCheck={setIsCheck} title={title} />
      <BulkActionDrawer ids={allId} title="ChatGroups" />
      <MainDrawer>
        <ChatGroupDrawer id={serviceId} />
      </MainDrawer>
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          ...
          <TableFooter>
            <Pagination
              ...
              label="ChatGroup Page Navigation"
            />
          </TableFooter>
        </TableContainer>
      ) : (
        <NotFound title="ChatGroup" />
      )}
    </>
  );
};
