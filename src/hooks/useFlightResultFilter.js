...
const useFlightResultFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newFlightResults] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newFlightResults.push(data[i].data);
    }
  };

  const handleUploadFlightResults = () => {
    if (newFlightResults.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      FlightResultServices.addAllFlightResults(newFlightResults)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        FlightResultServices.addAllFlightResults(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadFlightResults,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useFlightResultFilter;
