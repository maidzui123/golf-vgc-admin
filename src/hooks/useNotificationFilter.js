...
const useNotificationFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newNotifications.push(data[i].data);
    }
  };
  ...
  const handleUploadNotifications = () => {
    if (newNotifications.length < 1) {
      ...
      NotificationServices.addAllNotifications(newNotifications)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        NotificationServices.addAllNotifications(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
