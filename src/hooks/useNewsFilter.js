...
const useNewsFilter = (data) => {
    ...
    const handleOnDrop = (data) => {
        ...
        for (let i = 0; i < data.length; i++) {
            newNewss.push(data[i].data);
        }
    };

    const handleUploadNewss = () => {
        if (newNewss.length < 1) {
            notifyError("Please upload/select csv file first!");
        } else {
            // notifySuccess('CRUD operation disable for demo!');
            NewsServices.addAllNewss(newNewss)
                .then((res) => {
    ...
    const handleUploadMultiple = (e) => {
        if (selectedFile.length > 1) {
            ...
            if (validationData) {
                NewsServices.addAllNewss(selectedFile)
                    .then((res) => {
                        setIsUpdate(true);
                        setLoading(false);
                        notifySuccess(res.message);
