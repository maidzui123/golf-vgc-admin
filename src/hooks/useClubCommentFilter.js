...
const useClubCommentFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newClubComments] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newClubComments.push(data[i].data);
    }
  };

  const handleUploadClubComments = () => {
    if (newClubComments.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ClubCommentServices.addAllClubComments(newClubComments)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ClubCommentServices.addAllClubComments(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadClubComments,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useClubCommentFilter;
