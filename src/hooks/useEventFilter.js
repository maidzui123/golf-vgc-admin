...
const useEventFilter = (data) => {
  ...
  const handleUploadEvents = () => {
    if (newEvents.length < 1) {
      ...
      EventServices.addAllEvents(newEvents)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        EventServices.addAllEvents(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
