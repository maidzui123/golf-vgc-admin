...
const useCustomerClubFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerClubs] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerClubs.push(data[i].data);
    }
  };

  const handleUploadCustomerClubs = () => {
    if (newCustomerClubs.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerClubServices.addAllCustomerClubs(newCustomerClubs)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerClubServices.addAllCustomerClubs(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerClubs,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerClubFilter;
