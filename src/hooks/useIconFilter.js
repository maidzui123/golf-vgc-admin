...
const useIconFilter = (data) => {
  ...
  const handleUploadIcons = () => {
    if (newIcons.length < 1) {
      ...
      IconServices.addAllIcons(newIcons)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        IconServices.addAllIcons(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
