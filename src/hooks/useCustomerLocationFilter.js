...
const useCustomerLocationFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerLocations] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerLocations.push(data[i].data);
    }
  };

  const handleUploadCustomerLocations = () => {
    if (newCustomerLocations.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerLocationServices.addAllCustomerLocations(newCustomerLocations)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerLocationServices.addAllCustomerLocations(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerLocations,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerLocationFilter;
