...
const useSearchKeywordFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newSearchKeywords] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newSearchKeywords.push(data[i].data);
    }
  };

  const handleUploadSearchKeywords = () => {
    if (newSearchKeywords.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      SearchKeywordServices.addAllSearchKeywords(newSearchKeywords)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        SearchKeywordServices.addAllSearchKeywords(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadSearchKeywords,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useSearchKeywordFilter;
