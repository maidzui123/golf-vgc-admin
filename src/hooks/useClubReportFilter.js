...
const useClubReportFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newClubReports.push(data[i].data);
    }
  };

  const handleUploadClubReports = () => {
    if (newClubReports.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ClubReportServices.addAllClubReports(newClubReports)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ClubReportServices.addAllClubReports(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadClubReports,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useClubReportFilter;
