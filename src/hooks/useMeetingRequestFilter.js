...
const useMeetingRequestFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newMeetingRequests] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newMeetingRequests.push(data[i].data);
    }
  };

  const handleUploadMeetingRequests = () => {
    if (newMeetingRequests.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      MeetingRequestServices.addAllMeetingRequests(newMeetingRequests)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        MeetingRequestServices.addAllMeetingRequests(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadMeetingRequests,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useMeetingRequestFilter;
