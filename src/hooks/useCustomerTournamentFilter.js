...
const useCustomerTournamentFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerTournaments] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerTournaments.push(data[i].data);
    }
  };

  const handleUploadCustomerTournaments = () => {
    if (newCustomerTournaments.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerTournamentServices.addAllCustomertournaments(newCustomerTournaments)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerTournamentServices.addAllCustomerTournaments(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerTournaments,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerTournamentFilter;
