...
const useCustomerTournamentFavouriteFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerTournamentFavourites] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerTournamentFavourites.push(data[i].data);
    }
  };

  const handleUploadCustomerTournamentFavourites = () => {
    if (newCustomerTournamentFavourites.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerTournamentFavouriteServices.addAllCustomerTournamentFavourites(newCustomerTournamentFavourites)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerTournamentFavouriteServices.addAllCustomerTournamentFavourites(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerTournamentFavourites,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerTournamentFavouriteFilter;
