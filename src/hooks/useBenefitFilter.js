...
const useBenefitFilter = (data) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        BenefitServices.addAllBenefits(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadBenefits,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useBenefitFilter;
