...
const useCustomerRewardFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerRewards] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerRewards.push(data[i].data);
    }
  };

  const handleUploadCustomerRewards = () => {
    if (newCustomerRewards.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerRewardServices.addAllCustomerRewards(newCustomerRewards)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerRewardServices.addAllCustomerRewards(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerRewards,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerRewardFilter;
