...
const usePaymentFilter = (data) => {
  ...
  const handleUploadPayments = () => {
    if (newPayments.length < 1) {
      ...
      PaymentServices.addAllPayments(newPayments)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        PaymentServices.addAllPayments(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
