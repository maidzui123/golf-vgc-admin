...
const useLocationClubFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newLocationClubs] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newLocationClubs.push(data[i].data);
    }
  };

  const handleUploadLocationClubs = () => {
    if (newLocationClubs.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      LocationClubServices.addAllLocationClubs(newLocationClubs)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        LocationClubServices.addAllLocationClubs(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadLocationClubs,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useLocationClubFilter;
