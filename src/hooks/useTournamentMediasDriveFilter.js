...
const useTournamentMediasDriveFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newTournamentMediasDrives] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newTournamentMediasDrives.push(data[i].data);
    }
  };

  const handleUploadTournamentMediasDrives = () => {
    if (newTournamentMediasDrives.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      TournamentMediasDriveServices.addAllTournamentMediasDrives(newTournamentMediasDrives)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        TournamentMediasDriveServices.addAllTournamentMediasDrives(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadTournamentMediasDrives,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useTournamentMediasDriveFilter;
