...
const useRewardFilter = (data) => {
  ...
  const handleUploadRewards = () => {
    if (newRewards.length < 1) {
      ...
      RewardServices.addAllRewards(newRewards)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        RewardServices.addAllRewards(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
...
export default useRewardFilter;
