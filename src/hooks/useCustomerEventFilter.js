...
const useCustomerEventFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerEvents] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerEvents.push(data[i].data);
    }
  };

  const handleUploadCustomerEvents = () => {
    if (newCustomerEvents.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerEventServices.addAllCustomerEvents(newCustomerEvents)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerEventServices.addAllCustomerEvents(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerEvents,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerEventFilter;
