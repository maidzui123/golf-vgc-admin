...
const useReportNameFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newReportNames.push(data[i].data);
    }
  };

  const handleUploadReportNames = () => {
    if (newReportNames.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ReportNameServices.addAllReportNames(newReportNames)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ReportNameServices.addAllReportNames(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadReportNames,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useReportNameFilter;
