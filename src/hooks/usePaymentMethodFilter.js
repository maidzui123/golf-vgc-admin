...
const usePaymentMethodFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newPaymentMethods] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newPaymentMethods.push(data[i].data);
    }
  };

  const handleUploadPaymentMethods = () => {
    if (newPaymentMethods.length < 1) {
      ...
      PaymentMethodServices.addAllPaymentMethods(newPaymentMethods)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        PaymentMethodServices.addAllPaymentMethods(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadPaymentMethods,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default usePaymentMethodFilter;
