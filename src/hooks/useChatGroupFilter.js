...
const useChatGroupFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newChatGroups.push(data[i].data);
    }
  };

  const handleUploadChatGroups = () => {
    if (newChatGroups.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ChatGroupServices.addAllChatGroups(newChatGroups)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ChatGroupServices.addAllChatGroups(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadChatGroups,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useChatGroupFilter;
