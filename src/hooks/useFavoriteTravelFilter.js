...
const useFavoriteTravelFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newFavoriteTravels] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newFavoriteTravels.push(data[i].data);
    }
  };

  const handleUploadFavoriteTravels = () => {
    if (newFavoriteTravels.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      FavoriteTravelServices.addAllFavoriteTravels(newFavoriteTravels)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        FavoriteTravelServices.addAllFavoriteTravels(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadFavoriteTravels,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useFavoriteTravelFilter;
