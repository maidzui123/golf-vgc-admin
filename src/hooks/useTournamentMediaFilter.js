...
const useTournamentMediaFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newTournamentMedias] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newTournamentMedias.push(data[i].data);
    }
  };

  const handleUploadTournamentMedias = () => {
    if (newTournamentMedias.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      TournamentMediaServices.addAllTournamentMedias(newTournamentMedias)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        TournamentMediaServices.addAllTournamentMedias(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadTournamentMedias,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useTournamentMediaFilter;
