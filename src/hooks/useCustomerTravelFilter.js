...
const useCustomerTravelFilter = (data) => {
  const ajv = new Ajv({ allErrors: true });
  const { setLoading, setIsUpdate } = useContext(SidebarContext);

  const [newCustomerTravels] = useState([]);
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCustomerTravels.push(data[i].data);
    }
  };

  const handleUploadCustomerTravels = () => {
    if (newCustomerTravels.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CustomerTravelServices.addAllCustomerTravels(newCustomerTravels)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CustomerTravelServices.addAllCustomerTravels(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadCustomerTravels,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useCustomerTravelFilter;
