...
const useClubPostFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newClubPosts.push(data[i].data);
    }
  };

  const handleUploadClubPosts = () => {
    if (newClubPosts.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      ClubPostServices.addAllClubPosts(newClubPosts)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        ClubPostServices.addAllClubPosts(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
  ...
  return {
    ...
    serviceData,
    handleOnDrop,
    handleUploadClubPosts,
    handleRemoveSelectFile,
    handleUploadMultiple,
  };
};

export default useClubPostFilter;
