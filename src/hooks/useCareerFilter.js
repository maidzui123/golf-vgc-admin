...
const useCareerFilter = (data) => {
  ...
  const handleOnDrop = (data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      newCareers.push(data[i].data);
    }
  };

  const handleUploadCareers = () => {
    if (newCareers.length < 1) {
      notifyError("Please upload/select csv file first!");
    } else {
      // notifySuccess('CRUD operation disable for demo!');
      CareerServices.addAllCareers(newCareers)
        .then((res) => {
  ...
  const handleUploadMultiple = (e) => {
    if (selectedFile.length > 1) {
      ...
      if (validationData) {
        CareerServices.addAllCareers(selectedFile)
          .then((res) => {
            setIsUpdate(true);
            setLoading(false);
            notifySuccess(res.message);
